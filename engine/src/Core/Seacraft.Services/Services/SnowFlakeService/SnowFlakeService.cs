...
namespace Seacraft.Services
{
        ...
        : PeriodicHostedService, IAsyncDisposable
    {
        ...
        {
            this._cache = cache;
            this._distributedLockProvider = distributedLock;

            this.Name = options.Value.Name;
        ...
        public async Task<SnowFlake> CreateSnowFlakeAsync()
        {
            var @lock = this._distributedLockProvider.CreateLock(CacheLockKey);
            await using (await @lock.AcquireAsync())
            {
                var ip = long.Parse(LocalIp.Replace(".", "")); //1921680200
                this.MachineId = ip % 32; //0-31

                var machineId = await this.CreateMachineIdAsync(LocalIp, this.MachineId);
                if (machineId != null)
                {
                    this.MachineId = machineId.Value;
                }
        ...
        private async Task<bool> RegisterMachineAsync(long machineId, string localIp)
        {
            try
            {
                var key = this.MakeKey(machineId);

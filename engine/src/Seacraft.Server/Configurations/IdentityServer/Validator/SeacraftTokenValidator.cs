...
// The Seacraft licenses 'SeacraftTokenValidator.cs' file under the MIT license.
...
namespace Seacraft.Server.Configurations.IdentityServer.Validator
{

    public class SeacraftTokenValidator : ITokenValidator
    {
        ...
        public SeacraftTokenValidator(
            ...
            ISystemClock clock,
            ILogger<SeacraftTokenValidator> logger)
        {
            _options = options;
            _context = context;
            _clients = clients;
        ...
        private async Task<TokenValidationResult> ValidateJwtAsync(string jwt, IEnumerable<SecurityKeyInfo> validationKeys, bool validateLifetime = true, string audience = null)
        {
            ...
            {
                // 如果设置了IssuerUri优先用IssuerUri
                ValidIssuer = _options.IssuerUri ?? _context.HttpContext.GetIdentityServerIssuerUri(),
            ...
            try
            {
                ...
                // check if there is a reference_hash and return directly
                var referenceHash = id.FindFirst(Constant.Claims.ReferenceHash)?.Value;
                if (referenceHash.IsPresent())
                {
                    return await ValidateReferenceAccessTokenAsync(referenceHash);
                }
        ...
        private async Task<TokenValidationResult> ValidateReferenceAccessTokenAsync(string tokenHandle)
        {
            ...
            {
                if (_clients is SeacraftClientStore synyiClientStore && !(await synyiClientStore.IsActive(token.ClientId)))
                {
                    client = null;
                }
                else

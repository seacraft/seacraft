...
func (a *AppTemplateController) List(c *gin.Context) {
	log.L(c).Info("list app template function called.")
	var r metav1.ListOptions
	if err := c.ShouldBindJSON(&r); err != nil {
		core.WriteResponse(c, errors.WithCode(code.ErrBind, err.Error()), nil)
		return
	}
	tmps, err := a.srv.AppTemplates().List(c, r)
	if err != nil {
		core.WriteResponse(c, err, nil)
		return
	}
	core.WriteResponse(c, nil, tmps)
}
